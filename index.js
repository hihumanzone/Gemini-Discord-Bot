...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'respondtoall') {
    ...
    try {
      ...
      const outputUrl = await speechGen(text, language);
      if (outputUrl && outputUrl !== 'Output URL is not available.') {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            { name: '**Prompt**', value: `\`\`\`${text}\`\`\``, inline: false },
...
async function handleModalSubmit(interaction) {
  if (interaction.customId === 'custom-personality-modal') {
    ...
    try {
      ...
      const outputUrl = await speechGen(text, 'en');
      if (outputUrl && outputUrl !== 'Output URL is not available.') {
        const file = new AttachmentBuilder(outputUrl);
        const embed = new EmbedBuilder()
          .setColor(0x0099ff)
...
async function showSettings(interaction) {
  ...
  const actionRows = [];
  const allButtons = [clearButton, toggleChatButton, customPersonalityButton, removePersonalityButton, generateImageButton, imageToggleButton, userResponseToggle, generateAudioButton];
...
async function speechGen(text, language) {
  try {
